from API_Testing.books.requests.api_clients import get_token
from API_Testing.books.requests.orders import add_order, delete_order, get_orders, get_order, edit_order
...
class TestOrders:
    ...
    # def test_patch_invalid_order_id(self):
    #     id = add_order(self.token, 1, 'Mary').json()['orderId']
    #     r = edit_order(self.token, id, 'Marya')
    #     assert r.status_code == 204, 'code not ok'
    #     get = get_order(self.token, id)
    #     assert get.json()['customerName'] == 'Mary', 'update name not working'
    #     # cleanup
    #     delete_order(self.token, id)